...
<body>
	...
	<div class="forum">
		<header>
			...
			<button class="button2 post_topic" onclick="topic_guide()">发帖指南</button>
		</header>
		<main class="board">
		</main>
	</div>
...
<script>
	...
	last_page_button.addEventListener('click', function() {
		...
		if (page < 0) {
			...
			// 从URL进行跳转
			window.location.href = `./forum_list.php?fid=${fid}&page=${page}`
		}
	})
	...
	const previews_add = (tid, aids) => {
		...
		for (let i = 0; i < aids.length; i++) {
			var html = `<img src="./data/forums/${tid}/preview_${i}.jpg" onclick="fullscreen(this)" alt="图片加载失败">`
	...
	const add_topic = (forums) => {
		...
		for (n = 0; n < forums.length; n++) {
			...
			if (forums[n]['newest_reply']) {
				...
				var newest_content = forums[n]['newest_reply'];
			...
			var html = `
				<!-- 帖子 -->
				<div class="post tty" id="_${forums[n]['tid']}">
					<!-- 左部分帖子文字预览 -->
					<div class="post_info">
						<a href="./view_topic.php?tid=${forums[n]['tid']}&page=0" target="_blank" class="topic_title">${forums[n]['title']}</a><br>
						<span class="topic_auther">${forums[n]['auther']}</span>-<span class="date">${forums[n]['date']}</span><br>
						<span class="topic_content">${forums[n]['content'].slice(0, 40)}</span>
					</div>
					...
					<!-- 最新回复 -->
					<div class="new_post">
						<span class="post_num">${forums[n]['reply_count']}回复</span><br>
						<span class="post_date">${date}</span><br>
						<span class="post_content">${newest_content}</span>
					</div>
				</div>
			`
			...
			// 判断预览图是否存在
			if (forums[n]['preview']) {
				previews_add(forums[n]['tid'], forums[n]['preview'])
			}
		}
	}
